...
def sendText(toNum,text,name):
    ...
    message = client.messages.create(
        to="+"+str(toNum), 
        from_="+12563673601",
        body="Hello {}, {}: Do you like pizza? ".format(name,text))
...
#Label Message
label4 = tk.Label(win, bg = 'gold', wraplength = 500, font = ("Times New Roman",10), text = "Start Game, text reset to restart.").pack()

#Entry field Message
entry_field3 = tk.Entry(win, textvariable = y).pack()

#Makes a button
button1 = tk.Button(win, text = "BEGIN", cursor = 'heart', command = handle_request)
button1.pack()
